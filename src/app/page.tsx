...
export default function HomePage() {
  ...
  useEffect(() => {
    // 앱 진입 시 미로그인 상태라도 제출작 데이터 즉시 불러오기
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-gray-900 flex flex-col">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 md:p-6 space-y-6">
        {/* 미로그인 상태 시 안내 뱃지 배너 */}
        {!currentUser && (
          <div className="glass-card p-4 flex flex-col sm:flex-row items-center justify-between gap-3 border-blue-200 bg-blue-50/50">
            <div className="flex items-center gap-2 text-xs text-blue-900">
              <ShieldCheck className="w-4 h-4 text-blue-600 shrink-0" />
              <span>
                <strong>방문자 열람 모드:</strong> 현재 등록된 모든 심사 자료 및 원본 구글 드라이브 링크를 자유롭게 열람하실 수 있습니다. (점수 채점은 카카오/관리자 로그인 후 가능)
              </span>
            </div>
            <div className="flex items-center gap-2 shrink-0">
              <button
                onClick={() => setIsKakaoLoginModalOpen(true)}
                className="px-3.5 py-1.5 rounded-lg bg-[#FEE500] hover:bg-[#e6cf00] text-black font-bold text-xs shadow transition flex items-center gap-1.5"
              >
                카카오 로그인
              </button>
              <button
